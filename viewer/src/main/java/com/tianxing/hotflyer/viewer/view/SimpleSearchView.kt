 ...
 */
class SimpleSearchView @JvmOverloads constructor(private val mContext: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : FrameLayout(mContext, attrs), Filter.FilterListener {
    ...
    private fun initiateView() {
        ...
        mSuggestionsListView = mSearchLayout!!.findViewById<View>(R.id.suggestion_list) as ListView
    ...
    private fun startFilter(s: CharSequence) {
        if (mAdapter != null && mAdapter is Filterable) {
            (mAdapter as Filterable).filter.filter(s, this@SimpleSearchView)
        }
    }
...
//    }
    ...
    companion object {
        ...
        fun hide(view: View?, listener: AnimationUtil.AnimationListener) {
